...
LOCAL BACKEND     → Express.js (runs inside Electron main process)
DATABASE          → SQLite via better-sqlite3 (local, no server needed)
AUTH              → JWT + bcrypt
EMAIL             → Nodemailer (SMTP / Gmail OAuth2)
CERTIFICATES      → Puppeteer (HTML-to-PDF) or PDFKit
QR CODES          → qrcode npm package
FILE STORAGE      → Local filesystem via Electron app.getPath('userData')
