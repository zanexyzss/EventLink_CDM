...
async function sendEmail({ to, subject, html, attachments = [], type = 'general' }) {
  ...
  try {
    ...
    db.prepare('INSERT INTO email_log (recipient_email, subject, type, status) VALUES (?,?,?,?)')
...
async function sendCertificate(user, event, certificatePath) {
  return sendEmail({
    ...
    type: 'certificate',
    attachments: [{ filename: `Certificate_${user.full_name.replace(/ /g,'_')}.pdf`, path: certificatePath }],
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
...
async function bulkSendCertificates(attendees) {
  return Promise.allSettled(attendees.map(({ user, event, certificatePath }) => sendCertificate(user, event, certificatePath)));
}
    ...
    <body>
      <div class="cert">
        ...
        <div class="footer">
          ...
          <div class="sig-line"><div>System Administrator</div></div>
        </div>
      </div>
    </body>
    </html>
...
async function generateCertificate(user, event) {
  ...
  await page.setContent(getCertificateHTML(user, event), { waitUntil: 'networkidle0' });
